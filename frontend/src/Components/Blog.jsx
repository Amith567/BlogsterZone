            ...
            <div className='w-full  p-3'>
                <div className='rounded-md flex flex-col ' onClick={()=>navigate(`/blog/${blog.id}`)}>
                    <div className='bg-blue-900 rounded-t p-2'>
                        ...
                        <div className='text-xs text-gray-300 '><span className='underline cursor-pointer ' onClick={(e)=>{
                            e.stopPropagation()
                            navigate(`/${blog.author_id}/profile`)
                        }}>Author : {blog.author} </span>| Published : {blog.created_at} | Likes : {blog.like_count}</div>
                    </div>
